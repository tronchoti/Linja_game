...
class controlador:
    ...
    def __init__(self) -> None:
        ...
        Establece el turno inicial al jugador ROJO.
        '''
        self.Tb = Tablero()
        self.Tb.rellenar_tablero()
        self.alpha = -math.inf
        self.beta = math.inf
        self.turno = Color.RED
        self.num_recur = 0
    ...
    def seleccionar_movimiento(self, numMov: int = 1,  depth: int = 15, avanza: int = 1):
        ...
        if self.turno == Color.RED:
            ...
            if numMov == 1 or numMov == 3:
                if new_row == 0 or new_row == 7:
                    ...
                    pass
                self.seleccionar_movimiento(numMov+1, 2,num_mov_2)
            elif numMov == 2:
                if num_mov_2 == 0:
                    num_mov_2 = 1
                else:
    ...
    def MAX(self, tablero: Tablero, numMovs: int, tipo: int, depth:int, alpha: int, beta: int):
        ...
        # SI NO SE HA CUMPLIDO NINGUNA DE LAS CONDICIONES ANTERIORES, NODO NO TERMINAL
            ...
            # Obtenemos los movimientos posibles e iteramos sobre la lista
            for i in options:
                # Optimización para ahorrar el cálculo de algunas ramas. Ver README para más
                if last_row == None:
                    last_row = i[0]
    ...
    def IA2(self, numMovs: int, tipo: int, depth: int):
        '''Controladora de la IA
        
        Este metodo es el encargado de gestionar los movimientos de la IA
        y de llamar al metodo MAX para inciar la ejecucion de MINIMAX.

        Args:
            numMovs (int): 
                Numero de movimientos que puede realizar.
            tipo (int):
                Movimiento en el que se encuentra dentro de la jugada.
        
        '''
        print(f"Profundidad exploracion es {depth}")
        lista = self.MAX(self.Tb, numMovs, tipo, depth, self.alpha, self.beta)
        movs = lista[1]
        for i in reversed(movs):
